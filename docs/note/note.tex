...
the details we need and display them the way we want.

\subsection{Old Summary Web Page}
The summary web page of the old system was designed to vehiculate all the needed
informations in a single view (Figure~\ref{fig:old-summary}), we could reuse its
layout and infrastructure.  To evaluate the feasibility of this approach, we
produce, with the new system, partial build and test summary files that are
compatible with the expectations of the old system.
...
XML, with the possibility of a layout usable on small screens (smartphones).

The amount of work required to reproduce completely the old summary files will
be probably spent better in the extension of CDash or Jenkins, or even in a
