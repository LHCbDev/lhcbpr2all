...
time lbn-build --jobs 8 --timeout 18000 --build-id "${slot}.${slot_build_id}.${timestamp}" --artifacts-dir "${ARTIFACTS_DIR}" --rsync-dest "buildlhcb.cern.ch:${deploybase}/${slot_build_id}" --with-tests --submit ${deploy_opt} ${coverity_opt} ${config_file}
